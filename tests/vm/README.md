...
| `vm-11-reconcile-commit.sh` | read-only | `pearlite reconcile --commit` (ADR-0014) end-to-end. Phase A — `AI_AGENT=1` triggers `RECONCILE_REQUIRES_INTERACTIVE`. Phase B — `--adopt-all --commit-threshold 0` trips `RECONCILE_THRESHOLD_EXCEEDED`. Phase C — `q` inside a `script(1)` pty session aborts cleanly with `state.toml` byte-identical (skipped if `script(1)` is absent). Phase D — `--adopt-all` writes the `[[reconciliations]]` entry and populates `[adopted].pacman`. Read-only with respect to system state; sandbox is a tempdir. |
